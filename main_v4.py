...
def classify_expression(landmarks, img_w, img_h):
    ...
    - corner_avg : 입꼬리가 올라간 방향
    """

    # Mediapipe FaceMesh의 입 주변 주요 인덱스들
    ...
    # 입꼬리가 중심보다 위/아래인지 (y축: 위로 갈수록 값이 작음)
    left_diff = ly - center_y
    right_diff = ry - center_y
    corner_avg = (left_diff + right_diff) / 2.0
    # corner_avg < 0 이면 평균적으로 입꼬리가 중심보다 위쪽
    ...
    # 1) 크게 웃는 웃음: 입이 많이 벌어졌으면 그냥 SMILE
    if mouth_ratio > BIG_SMILE_RATIO:
        return "SMILE"

    # 2) 입 다문 미소: 입은 조금만 벌어졌지만, 입꼬리가 위로 가 있을 때
    if SMALL_SMILE_RATIO < mouth_ratio <= BIG_SMILE_RATIO and corner_avg < -0.5:
        return "SMILE"

    # 3) 거의 안 벌려진 입 + 입꼬리도 평평/살짝 아래 → NEUTRAL
    if mouth_ratio < NEUTRAL_MOUTH_RATIO and corner_avg >= -1.0:
        return "NEUTRAL"
